...
```
              ┌───────────────┐
   用户请求  → │  Supervisor  │ ← 分派任务·管理共享状态·强制边界·最终整合
              └──┬─────┬────┬─┘
        ┌────────┘     │    └──────────┐
   ┌────▼────┐   ┌─────▼──────┐   ┌────▼─────┐
   │ Coder   │   │ Researcher │   │ Reviewer │  ← 专门 Worker
   └─────────┘   └────────────┘   └──────────┘
```
...
```
① Handoff(交接)            A ──控制+载荷──▶ B
   一方说「接下来交给你」连控制一起交出

② Supervisor/Worker(中央集权)   Supervisor
   主管分派·聚合。                   ┌──┼──┐
   交流总是经由主管                W1  W2  W3
   (Worker 之间不直接对话)

③ 共享黑板(Blackboard)     ┌─ 共享 state ─┐
   大家读写同一处                A   B   C

④ Peer-to-Peer / Swarm      A ⇄ B ⇄ C
   智能体之间直接往来(无中央)

⑤ Debate/Consensus(合议)    A·B·C 辩论 → 投票得结论
```
