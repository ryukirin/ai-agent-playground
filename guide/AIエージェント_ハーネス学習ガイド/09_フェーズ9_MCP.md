...
Python の公式 SDK(`mcp`)で、フェーズ 3 の「電卓」を MCP サーバとして公開します。

```python
# pip install mcp
# server.py ── 電卓を提供する MCP サーバ
...
登録後、AI 側からは `calculate` ツールが見えるようになり、フェーズ 3 で手で定義したのと同じように呼び出せます ── ただし**今後どのクライアントからでも再利用できる**点が決定的に違います。
...
4. **【実装】** 上の `server.py` を実際に動かし、`read_file` ツールと「現在時刻を返す resource」を追加せよ。可能なら Claude Desktop か Cursor に登録して呼び出せ。
