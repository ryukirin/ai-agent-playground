...
> 各章は「解説文 + 例コード + 課題」で構成。コード例は Python(Anthropic SDK)を基本に、本質がわかる最小構成で示す。
...
```
┌─────────────────────────────────────────┐
│  ハーネス (Harness) = エージェントの足回り        │
│  ┌───────────────────────────────────┐  │
│  │  モデル (LLM) = エンジン・頭脳            │  │  ← 賢いが、文章を生成するだけ
│  └───────────────────────────────────┘  │
│  ツール実行・コンテキスト管理・権限制御・メモリ・ループ │  ← ここが「エージェント化」する層
└─────────────────────────────────────────┘
```
